...
int main()
{
	...
	//Entrada da quantidade de areas
	cin >> areas;
	for(int i = 0; i < areas; i++)
	{
		...
		list = Graham_scan(v, pontos);
		cout << "Parte " << i+1 << endl;
		while(!list.empty())
		{
			cout << "x: " << list.back().x ;
			cout <<", y: " << list.back().y << endl;
			list.pop_back();
		}
		area = area_poligono(v,pontos);
		// cout <<"Area da Parte " << i+1 << ": " << area << endl;
	}
