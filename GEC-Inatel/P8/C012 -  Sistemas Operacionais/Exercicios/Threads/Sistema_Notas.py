...
def thread1():
    ...
    for i in n:
        ...
        soma_aux = prova1[i] + kahoot1[i] + trabalho1[i] + trabalho2[i] + \
            lista1[i] + lista2[i] + lista3[i] + lista4[i] + lista5[i]
        np1.append(soma_aux)
        print(soma_aux)
...
def thread2():
    ...
    for i in n:
        ...
        soma_aux_2 = prova2[i] + kahoot2[i] + trabalho3[i] + \
            trabalho4[i] + lista6[i] + lista7[i] + lista8[i] + seminario[i]
...
for i in n:
    ...
    print(
        f'As notas do aluno {i + 1} foram:   NP1 = {np1[i]}    NP2 = {np2[i]}    Média = {media[i]}    Situação = {status[i]}.')
